...
class BEA_Net(nn.Module):
    def __init__(self, in_channels=1, num_classes=2, base_num_features=32, n_layer=5, convolutional_upsampling=True,
                 norm_cfg='IN', activation_cfg='LeakyReLU'):
        super(BEA_Net, self).__init__()
...
if __name__ == '__main__':
    # https://blog.csdn.net/weixin_42489272/article/details/135125920   JBHI
    # x = torch.randn(2, 1, 256, 256).cuda(0)
    # 加载bea-net.pth权重
    model = BEA_Net().cuda(0)
    checkpoint = torch.load('bea-net.pth')
    print(checkpoint.keys())
    model.load_state_dict(checkpoint['network_weights'])   
    x = np.load('test_input\\0021092.npy')
    result = np.zeros(x.shape)
    for i in range(x.shape[1]):
        print("predicting slice:", i)
        x_ = torch.tensor(x[:, i]).cuda(0)
        x_ = F.interpolate(x_.unsqueeze(0), size=(256, 256), mode='bilinear', align_corners=False)
        out = model(x_)
        out = F.interpolate(out, size=(x.shape[-2], x.shape[-1]), mode='bilinear', align_corners=False).squeeze(0)
    ...
    print("Dice", dice)
    # # model = BEA_Net(n_layer=5, convolutional_upsampling=True, base_num_features=32).cuda(1)
    ...
    nib.save(nii, './test_output/bea_net_result.nii.gz')
